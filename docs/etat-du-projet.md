...
| Interface, le reste de la revue | Deux constats retenus et non traités au 12 septembre : il n'existe aucune échelle de texte commune (seize tailles, deux vocabulaires, un corps à 11 px sur des écrans regardés à deux mètres) ; une carte du salon ne dit toujours ni depuis quand la salle tourne, ni combien de places restent (qui est là, elle le dit depuis le 12 septembre). La colonne du menu XMB est désormais bornée (fenêtre de découpe au-dessus du croisement, garde de 72 px au-dessus du pied : trois entrées visibles au-dessus du curseur, huit sur quatorze au lieu de six, plus aucun contact avec la légende du pied). Mais la capture a RÉVÉLÉ un défaut plus ancien, non corrigé : l'entrée située juste au-dessus de la sélection se superpose au libellé du rayon (« volume » s'imprime sur « RÉGLAGES · SON · 7/14 »). Il préexiste à ce travail, aucune capture n'étant jamais descendue au septième rang d'une liste de quatorze. Le corriger demande de trancher entre « des entrées visibles au-dessus » et « aucun contact avec la bande des rayons », ce qui change le caractère du menu. |
| Pilotes et leur adresse | Relevé refait deux fois avant d'être juste, et la méthode compte autant que le chiffre : compter tout fichier de `spikes/m3-browser-drive` qui appelle `.goto(`, puis regarder s'il accepte `process.argv[2]`, `NEL3AB_URL`, ou importe `ROOM_URL`. Résultat au 12 septembre au soir : **61** pilotes ouvrent une page, **11** n'ont aucun moyen de changer d'adresse. Les deux premiers relevés (« vingt fichiers dont quatre en dur », puis « trente-sept dont six ») cherchaient `localhost:8100` littéralement et rataient tout pilote cassé autrement, dont `padmenu.mjs`, qui a pourtant une recette. Corrigés et exercés : `layout`, `contraste`, `library`. Corrigés et NON exercés : `flood`, `panel`, `polite`, `sonde`, `stir`. Restent les 11 sans échappatoire, à reprendre un par un contre une vraie salle. |

...
| Interface, le reste de la revue | Deux constats retenus et non traités au 12 septembre : il n'existe aucune échelle de texte commune (seize tailles, deux vocabulaires, un corps à 11 px sur des écrans regardés à deux mètres) ; une carte du salon ne dit toujours ni depuis quand la salle tourne, ni combien de places restent (qui est là, elle le dit depuis le 12 septembre). Un troisième est à moitié traité : le rayon du menu XMB annonce désormais le rang et le total, donc on sait que la liste continue, mais la colonne n'est toujours bornée ni en haut ni en bas. |
| Pilotes restés sur le port 8100 | Vingt fichiers de `spikes/m3-browser-drive` mentionnent encore `localhost:8100`, qui n'existe plus depuis que les salles ont leurs propres ports. Seize l'ont en repli derrière `process.argv[2]` et une recette qui passe une adresse suffit ; quatre l'écrivaient en dur (`polite.mjs`, `stir.mjs`, `panel.mjs`, et `layout.mjs` avant sa correction du 12 septembre). Les exercer un par un contre une vraie salle, puis leur donner une adresse en argument. |
